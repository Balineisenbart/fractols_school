...
void	my_mlx_pixel_put(t_image *image, int x, int y, int color)
{
	...
	dst = image->addr_ptr + (y * image->line_length + x * (image->pixel_bits
				/ 8));
...
void	init_configuration(t_config *config, char *arg)
{
	...
	config->mlx = mlx_init();
	config->window = mlx_new_window(config->mlx, WIN_SIZE, WIN_SIZE,
			WINDOW_STATEMENT);
	config->image.img_ptr = mlx_new_image(config->mlx, WIN_SIZE, WIN_SIZE);
	config->image.addr_ptr = mlx_get_data_addr(config->image.img_ptr,
